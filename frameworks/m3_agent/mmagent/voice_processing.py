...
VOICE_EMBEDDING_DEVICE = torch.device(
    os.getenv("M3_VOICE_EMBEDDING_DEVICE", "cuda" if torch.cuda.is_available() else "cpu")
)
...
embedding_model.to(VOICE_EMBEDDING_DEVICE)
...
def get_embedding(wav):
    ...
    def compute_embedding(wav_file, save=True):
        ...
        feat = feature_extractor(wav).unsqueeze(0).to(VOICE_EMBEDDING_DEVICE)
        with torch.no_grad():
...
def process_voices(video_graph, base64_audio, base64_video, save_path, preprocessing=[]):
    ...
    def diarize_audio(base64_audio, base64_video, filter=None):
        backend = os.getenv("M3_VOICE_BACKEND", "gemini").strip().lower()
        if backend == "local":
            from .voice_processing_local import diarize_audio_local

            return diarize_audio_local(base64_audio, filter=filter)
    ...
    try:
        ...
        asrs = diarize_audio(base64_audio, base64_video, filter=filter_duration_based)
