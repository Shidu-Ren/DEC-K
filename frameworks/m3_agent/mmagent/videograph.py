...
class VideoGraph:
    ...
    def refresh_equivalences_dev_one_to_one(self):
        """DEV mode: keep only strict face<->voice one-to-one equivalence pairs.

        Rules:
        - Only semantic nodes formatted as "Equivalence: <face_x>, <voice_y>" are considered.
        - Keep at most one voice per face and at most one face per voice.
        - Greedy selection by edge score, then by clip timestamp and semantic node id.
        """
        parent = {}
        rank = {}
        ...
        for node_id, node in self.nodes.items():
            ...
            if len(contents) == 0:
                continue
            caption = contents[0]
            if not isinstance(caption, str) or not caption.lower().startswith("equivalence"):
                continue

            entities = parse_video_caption(self, caption)
            face_ids = [entity_id for entity_type, entity_id in entities if entity_type == "face"]
            voice_ids = [entity_id for entity_type, entity_id in entities if entity_type == "voice"]

            # Strict mode accepts only one face and one voice in each equivalence.
            if len(face_ids) != 1 or len(voice_ids) != 1:
                continue
            ...
            if (
                face_id not in self.nodes
                or voice_id not in self.nodes
                or self.nodes[face_id].type != "img"
                or self.nodes[voice_id].type != "voice"
            ):
                continue

            edge_score = (
                float(self.edges.get((node_id, face_id), 0.0))
                + float(self.edges.get((node_id, voice_id), 0.0))
            )
            candidates.append((edge_score, _as_timestamp(node), node_id, face_id, voice_id))

        # High-score mappings first; stable tie-break by earlier clip then node id.
        candidates.sort(key=lambda x: (-x[0], x[1], x[2]))

        used_faces = set()
        used_voices = set()
        selected_pairs = []
        for _, _, _, face_id, voice_id in candidates:
            if face_id in used_faces or voice_id in used_voices:
                continue
            used_faces.add(face_id)
            used_voices.add(voice_id)
            selected_pairs.append((face_id, voice_id))

        for face_id, voice_id in selected_pairs:
            union(face_id, voice_id)
        ...
        for node_id in self.nodes:
            if self.nodes[node_id].type not in ["img", "voice"]:
                continue
            root = find(node_id)
            tag = f"face_{node_id}" if self.nodes[node_id].type == "img" else f"voice_{node_id}"
            if root not in root_to_character:
                root_to_character[root] = f"character_{character_count}"
                character_count += 1
            character = root_to_character[root]
        ...
        self.reverse_character_mappings = reverse_character_mappings
        logger.info(
            "DEV one-to-one equivalence: selected %s face-voice pairs, assigned %s characters",
            len(selected_pairs),
            character_count,
        )
