...
from .prompts import (
    ...
    prompt_generate_memory_with_ids_sft_dev_equivalence,
)
...
def process_memory_records(video_graph, memories, clip_id, type='episodic'):
    """Insert or update pre-embedded memory records in the graph.

    Args:
        video_graph: Target VideoGraph.
        memories: Iterable of {"contents": [text], "embeddings": [embedding]} dicts.
        clip_id: Global clip id / timestamp to attach to the text node.
    ...
    update_video_graph(video_graph, normalized_memories, type)
...
def process_memories(video_graph, memory_contents, clip_id, type='episodic'):
    ...
    process_memory_records(video_graph, memories, clip_id, type)
