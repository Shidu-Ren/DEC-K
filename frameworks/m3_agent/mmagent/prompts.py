...
prompt_generate_memory_with_ids_sft_dev_equivalence = """[DEV MODE: STRICT EQUIVALENCE RULES]
- In "high_level_conclusions", an equivalence line is optional.
- If you output equivalence, ONLY use this exact format: "Equivalence: <face_x>, <voice_y>".
- Do NOT output face-face or voice-voice equivalence.
- Each <face_x> can appear in at most one equivalence line.
- Each <voice_y> can appear in at most one equivalence line.
- If the evidence is weak or ambiguous, do not output equivalence for that case."""
...
prompt_generate_robot_dev_clip_detail = """You are given:
1. The original retrieved video clip.
2. Face and voice features extracted for this clip.
3. Clip memories already stored for this clip, which may contain stable character identities.

Your task is to generate one detailed, grounded clip description that helps question answering.

Strict requirements:
- Use the provided IDs exactly as given. Do not invent, rename, or merge IDs.
- Use the video for visual evidence.
- Use the voice features for spoken content and speaker-linked dialogue cues.
- Use the clip memories as prior context, but do not simply copy them verbatim.
- Add concrete grounded details that are useful for question answering: who appears, actions, dialogue, object states, locations, temporal order, and fine visual details.
- Prefer details that are directly visible in the clip or directly supported by the provided voice features.
- If a clip memory adds useful identity or scene context and does not conflict with the clip or voice features, you may use it.
- Prefer details that refine, disambiguate, or extend the existing clip memories instead of repeating them word-for-word.
- Do not mention the words "memory graph", "intermediate outputs", or "downstream model" in the answer.
- Do not invent facts that are not supported by the clip or the provided context.
- Write a single coherent paragraph, not a list.
- Make the paragraph detailed and information-dense rather than brief.

Return only the paragraph text, without JSON, bullet points, or extra explanation.
"""
