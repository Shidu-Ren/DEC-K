...
from mmagent.voice_processing_speaker import process_voices_speaker_aware
from mmagent.memory_processing_qwen import (
    process_memories,
    generate_memories_audio_only_speaker_aware,
)
...
def process_segment(
    ...
    id2voices = process_voices_speaker_aware(
        video_graph,
        base64_audio,
        base64_video=None,
    ...
    episodic_memories, semantic_memories = generate_memories_audio_only_speaker_aware(
        id2voices,
        clip_path,
    )
...
def streaming_process_video(
    ...
    clips = sorted(glob.glob(sample["clip_path"] + "/*"))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Merge adjacent same-speaker turns when temporal gap <= this value.",
    )
    parser.add_argument(
        "--min_speaker_confidence",
        type=float,
        default=0.0,
        help="Drop utterances below this speaker confidence during post-processing.",
    )
