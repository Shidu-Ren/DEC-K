...
def eval_minerva(folder_path, qa_data):
    categories = ['Object Recognition', 'Counting', 'Temporal Reasoning', 'State Changes', 'Numerical Reasoning', 'Counterfactual', 'Event Occurence', 'Listening', 'Cause and Effect', 'Situational Awareness', 'Spatial Perception', 'Reading', 'Goal Reasoning']
    ...
    for filename in os.listdir(folder_path):
        ...
        results[example_anno['question_type']]['num_total'] += 1
        if extract_characters_regex(example['response']) == example['answer']:
            results[example_anno['question_type']]['num_corrects'] += 1
            num_corrects += 1
        num_total += 1
    accuracy_list = []
    for cat in results:
